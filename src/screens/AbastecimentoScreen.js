...
export default function AbastecimentoScreen() {
    ...
    async function handleSalvar() {
        if (!motoSelecionada) return Alert.alert('Erro', 'Selecione uma moto.');
        ...
        if (isNaN(litrosFloat) || isNaN(precoFloat) || isNaN(kmInt)) {
            return Alert.alert('Erro', 'Digite valores numéricos válidos.');
        }
    ...
    return (
        ...
        >
            ...
            >
                ...
                {historico.length === 0 ? (
                    <Text style={styles.subtitle}>Nenhum abastecimento cadastrado.</Text>
                ) : (
                    historico.map(item => (
                        <View
                            key={item.id}
